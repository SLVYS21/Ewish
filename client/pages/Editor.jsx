...
/* ─── Guided steps ────────────────────────────────────────────── */
const STEPS = [
  { id: 'message', n: 1, title: 'Le message', sub: 'Texte & photos',    color: '#be185d', soft: '#fff1f6', accent: '#fbcfe8' },
  { id: 'look',    n: 2, title: 'Le look',    sub: 'Couleurs & fond',   color: '#4f46e5', soft: '#f0f0ff', accent: '#c7d2fe' },
  { id: 'extras',  n: 3, title: 'Les extras', sub: 'Options bonus',     color: '#047857', soft: '#f0fdf4', accent: '#a7f3d0' },
...
export default function Editor() {
  ...
  const handlePublish = async () => {
    setPublishing(true);
    try {
      await updatePublication(id, { data, style: { ...style, backgrounds }, decorations, jarConfig, widgets });
  ...
  const renderStepContent = () => {
    switch (activeStep) {
      ...
      case 'look':
        return (
          <div>
            <div className={styles.lookSubTabs}>
              {lookSubTabs.map(t => (
                <button
                  key={t.id}
                  className={`${styles.lookSubTab} ${lookSubTab === t.id ? styles.lookSubTabActive : ''}`}
                  onClick={() => setLookSubTab(t.id)}
                  style={lookSubTab === t.id ? { background: currentStep.soft, color: currentStep.color, borderColor: currentStep.accent } : {}}
                >
                  {t.label}
                </button>
              ))}
            </div>
            {lookSubTab === 'couleur' && <StyleTab style={style} onChange={handleStyleChange} />}
            {lookSubTab === 'fond'    && <BackgroundTab templateName={pub.templateName} backgrounds={backgrounds} onChange={handleBackgroundsChange} />}
            {lookSubTab === 'anim'    && (
              <>
                {showDeco     && <DecoTab decorations={decorations} onChange={handleDecorationsChange} />}
                {showConfetti && <ConfettiTab confettiType={confettiType} onChange={handleConfettiChange} iframeRef={iframeRef} />}
              </>
            )}
            {lookSubTab === 'photos' && showPhotos && <PhotoLayoutTab transforms={photoTransforms} onChange={handlePhotoTransformsChange} />}
          </div>
        );
      ...
      case 'extras':
        return (
          <div className={styles.extrasList}>
            ...
            {showWishes && (
              <AccordionCard icon={MailOpen} title="Vœux" sub="Messages collectifs" color="#047857">
      ...
      case 'share':
        return (
          <div className={styles.shareStep}>
            <div className={`${styles.shareHero} ${pub?.published ? styles.shareHeroPublished : styles.shareHeroReady}`}>
              <div className={styles.shareHeroEmoji}>{pub?.published ? '🎉' : '🚀'}</div>
              <div className={styles.shareHeroTitle}>{pub?.published ? 'Votre carte est en ligne !' : 'Prêt à publier ?'}</div>
              <div className={styles.shareHeroSub}>{pub?.published ? 'Partagez le lien ci-dessous' : 'Publiez pour obtenir votre lien de partage'}</div>
            </div>
            ...
            <button className={styles.sharePublishBtn} onClick={handlePublish} disabled={publishing}>
              {publishing
                ...
                : pub?.published ? 'Mettre à jour' : '✨ Publier ma création'}
            </button>
            ...
            {STEPS.map((step, i) => {
              ...
              return (
                <button
                  ...
                  style={isActive ? { borderBottomColor: step.color } : {}}
                >
                  <span
                    className={styles.stepNumCircle}
                    style={{
                      background: isActive ? step.color : isDone ? '#10b981' : undefined,
                      color:      (isActive || isDone) ? '#fff' : undefined,
